...
*
* description : allows the user to change their profile info 
*
* purpose: if the user needs to change thier password, update contact info or entered it 
* incorrectly the first time, this page provides a way to update that information
*
* input : database.php
*
* processing : 
...
* 3. prints html elements for the nav and form for updating user info
* 4. uses a select statment to pre fill the fields with existing data from that record 
* 5. sends the POST data back to this page and checks for validity 
* 6. if valid updates the record with the dat and returns to login screen to re enter login data 
* 7. else prinst error messages 
* 
* output :prints the html nav elements as well as the form for updating user info 
* with fields pre filled out from a select statment 
*
...
*/
	...
	if ( !empty($_POST)) {
		...
		if (!empty($results))
		{
			if ($results['userName'] != $_SESSION['gigsUserName'])
			{
				$valid = false;	
		...
		if ($valid) {
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE users set userName = ?, password = ?, email = ?, city = ?, state = ?, zip = ? WHERE userId = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($userName,$passwd,$email,$city,$state,$zip,$userId));
			Database::disconnect();
...
<head>
    ...
    <title>Gigs|Update User Profile</title>
