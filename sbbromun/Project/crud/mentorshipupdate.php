...
* from the database table(Mentorships) and allows adding new records.

* input : user input
*
* precondition : database with tables: Players, Mentors, Mentorships
*	User clicked Update on the mentorshipindex.php page.
* postcondition: update gui made or record modified to Mentorships table.
* *******************************************************************
*/
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE Mentorships  set playerID = ?, teacherID = ?, roleToTrain =? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($playerID,$teacherID,$roleToTrain,$id));
			Database::disconnect();
			header("Location: mentorshipindex.php");
		}
	} else {
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * FROM Mentorships where id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
		$data = $q->fetch(PDO::FETCH_ASSOC);
		$playerID = $data['playerID'];
		$teacherID = $data['teacherID'];
		$roleToTrain = $data['roleToTrain'];
		Database::disconnect();
	}
?>
...
<body>
    <div class="container">
    			...
    			<div class="span10 offset1">
	    			...
	    			<form class="form-horizontal" action="mentorshipupdate.php?id=<?php echo $id?>" method="post">
					...
					<!-- Uses select box to ensure only avaialble teachers chosen.-->
